...
% link:http://www.psi.toronto.edu/index.php?q=affinity%20propagation
...
if nargin<2 error('Too few input arguments');
    ...
    maxits=1000; convits=100; lam=0.9; plt=0; details=0; nonoise=0;TH=0.1;% 设置聚类中心阈值，原来为0
...
while ~dn
    ...
    % 迭代停止条件：近convits次迭代中选择的聚类中心稳定或者迭代次数达到上限值maxits（convits为局部决策稳定的大小，由迭代次数规定）
    E=((diag(A)+diag(R))>TH); e(:,mod(i-1,convits)+1)=E; K=sum(E);
...
I=find((diag(A)+diag(R))>TH); K=length(I); % Identify exemplars
if K>0
